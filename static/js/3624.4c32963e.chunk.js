"use strict";
(self["webpackChunknews"] = self["webpackChunknews"] || []).push([[3624],{
...
/* harmony export */   "default": () => (/* binding */ G2PlotCustomConfig)
/* harmony export */ });
...
function G2PlotCustomConfig(props){var _controller$getConfig;const{controller}=props;const config=(_controller$getConfig=controller.getConfig())===null||_controller$getConfig===void 0?void 0:_controller$getConfig.style;const codeRef=(0,react__WEBPACK_IMPORTED_MODULE_0__.useRef)((config===null||config===void 0?void 0:config.customCode)||'');const onFieldChange=fieldChangeData=>{const{id,data,dataFragment}=fieldChangeData;if(id==='customCode'){codeRef.current=data;return;}if(id==='refreshChart')controller.update({style:{customCode:codeRef.current}});else controller.update(dataFragment);};const schema={key:'style',type:'grid',children:[{type:'card-panel',label:'G2Plot代码',tip:'请参考G2Plot官方文档复制粘贴并调试代码',config:{contentStyle:{padding:0}},children:[{id:'customCode',type:'code-editor',value:codeRef.current,config:{language:'javascript',height:600}}]},{type:'button',id:'refreshChart',config:{children:'刷新图表',style:{width:'100%'}}}]};return/*#__PURE__*/(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_2__.jsx)(_json_schema_LCGUI__WEBPACK_IMPORTED_MODULE_1__.LCGUI,{schema:schema,onFieldChange:onFieldChange});}
