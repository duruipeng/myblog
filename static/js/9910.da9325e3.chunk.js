"use strict";
(self["webpackChunknews"] = self["webpackChunknews"] || []).push([[9910,57,6316,274],{
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   BaseImageStyleConfig: () => (/* binding */ BaseImageStyleConfig)
/* harmony export */ });
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(48331);
...
const BaseImageStyleConfig=_ref=>{let{controller}=_ref;const{style}=controller.getConfig();const{type,onLineUrl,localUrl,opacity}=style;const[,setCount]=(0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(0);const onFieldChange=fieldChangeData=>{const{id,data,dataFragment,reRender}=fieldChangeData;if(id==='localUrl'){const{hash,url}=data;controller.update({style:{localUrl:url,hash}});}else{controller.update(dataFragment);}if(reRender)setCount(count=>count+1);};const schema={key:'style',type:'grid',config:{columns:2},children:[{key:'type',type:'radio',label:'来源',value:type,reRender:true,config:{options:[{label:'本地',value:'local'},{label:'在线',value:'online'}],containerStyle:{gridColumn:'1/3'}}},{key:'onLineUrl',rules:"{type} === 'online'",type:'input',label:'链接',value:onLineUrl,config:{containerStyle:{gridColumn:'1/3'}}},{id:'localUrl',key:'localUrl',rules:"{type} === 'local'",type:'image-upload',label:'上传',value:localUrl,config:{accept:'image/*',size:3,containerStyle:{gridColumn:'1/3'}}},{key:'opacity',type:'number-input',label:'透明度',value:opacity,config:{min:0,max:1,step:0.1}}]};return/*#__PURE__*/(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_2__.jsx)(_json_schema_LCGUI__WEBPACK_IMPORTED_MODULE_1__.LCGUI,{schema:schema,onFieldChange:onFieldChange});};
...
/* harmony import */ var _BaseImageComponent__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(30057);
...
;// CONCATENATED MODULE: ./src/pages/Charts/comps/lc/base-image/BaseImageDefinition.ts
const BaseInfo=/*#__PURE__*/react.lazy(()=>__webpack_require__.e(/* import() */ 2049).then(__webpack_require__.bind(__webpack_require__, 34430)));const FilterConfig=/*#__PURE__*/react.lazy(()=>__webpack_require__.e(/* import() */ 9057).then(__webpack_require__.bind(__webpack_require__, 31438)));class BaseImageDefinition extends AbstractDesignerDefinition["default"]{getBaseInfo(){return{compName:"基础图片",compKey:"BaseImage",categorize:"media"};}getChartImg(){return baseImage_namespaceObject;}getController(){return BaseImageController["default"];}getInitConfig(){return{base:{id:"",name:'基础图片',type:'BaseImage'},style:{type:'online',localUrl:'',onLineUrl:'',opacity:1},filter:{enable:false,blur:0,brightness:1,contrast:1,opacity:1,saturate:1,hueRotate:0}};}getMenuList(){return super.getMenuList().filter(item=>item.key!=='theme'&&item.key!=='data');}getMenuToConfigContentMap(){return{base:BaseInfo,style:BaseImageConfig.BaseImageStyleConfig,filter:FilterConfig};}getEventList(){const events=super.getEventList();return events.concat([{id:"click",name:"点击时"}]);}}
